...
void URBasicAttack_Archer::ActivateSkill_Implementation()
{
	...
	float AttackRange = 3000.0f; // 궁수의 사정 거리
	...
	for (int32 i = 0; i < NumArrows; ++i)
	{
		...
		for (const FHitResult& Hit : HitResults)
		{
			...
			{
				// 데미지 적용 로직
				UE_LOG(LogTemp, Log, TEXT("궁수 화살 %d 공격: %s에게 총 %.2f 데미지."), i + 1, *HitActor->GetName(), GetTotalDamage());
